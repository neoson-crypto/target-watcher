...
const baba = '0xe831c8903de820137c13681e78a5780afddf7697';
...
cron.schedule('*/10 * * * *', async () => {
    ...
    const ethBalance = await ethContract.methods.earned(baba).call();

    const nyanContract = new web3.eth.Contract(StakeABI, nyanStakeAddr);
    const stakedBalance = await nyanContract.methods.balanceOf(baba).call();
    const earnedBalance = await nyanContract.methods.earned(baba).call();
    ...
    const msg = `ETH池(未收割): ${Number(web3.utils.fromWei(ethBalance)).toFixed(6)} NYAN\n`
        + `单池(已存): ${Number(web3.utils.fromWei(stakedBalance)).toFixed(6)} NYAN\n`
        + `单池(未收割): ${Number(web3.utils.fromWei(earnedBalance)).toFixed(6)} NYAN\n\n`
